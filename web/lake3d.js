...
window.Lake3D = (() => {
  ...
  function mount(host, waiting, waterName='月影湖') {
    ...
    const craft=window.SurfaceCraft?.(T,renderer);if(craft)scene.environment=craft.environment;
    ...
    const waterGeometry=new T.PlaneGeometry(240,240,240,240),waterVertices=waterGeometry.attributes.position;
    for(let i=0;i<waterVertices.count;i++){for(const axis of ['X','Y']){const v=waterVertices['get'+axis](i);waterVertices['set'+axis](i,Math.sign(v)*Math.pow(Math.abs(v)/120,1.35)*120);}}waterGeometry.computeVertexNormals();
    const water=new T.Mesh(waterGeometry,new T.ShaderMaterial({extensions:{derivatives:true},uniforms,transparent:true,depthWrite:false,vertexShader:`uniform float waveScale;uniform float time;uniform float impact;uniform vec2 center;varying vec3 wp;varying vec3 normalW;
      float wave(vec2 p){float t=time;float w=sin(p.x*.67+t*.72)*.047+sin(p.y*.94-t*.65)*.035+sin(p.x*1.7+p.y*1.3+t*.9)*.019;float d=distance(p,center),age=t-impact;float ring=sin(d*11.-age*8.)*exp(-pow(d-age*2.2,2.)*1.8)*exp(-age*.55);return w*waveScale*.68+(age>0.?ring*.18:0.);}
      ...
      void main(){vec2 p=wp.xz;float fade=1.-smoothstep(18.,95.,length(wp-cameraPosition));float r1=dot(p,vec2(.87,.49))*7.3+time*1.2;float r2=dot(p,vec2(-.32,.95))*12.7-time*.93;float r3=dot(p,vec2(.98,-.21))*22.+time*1.7;vec2 grad=vec2(.87,.49)*cos(r1)*.035+vec2(-.32,.95)*cos(r2)*.018+vec2(.98,-.21)*cos(r3)*.008*fade;vec2 drift=flow*time*.35;float micro=detail(p*3.2+drift);float mx=detail((p+vec2(.025,0.))*3.2+drift)-micro;float mz=detail((p+vec2(0.,.025))*3.2+drift)-micro;float footprint=max(length(dFdx(p)),length(dFdy(p)));float microFade=1.-smoothstep(.025,.18,footprint);grad=grad*.52+vec2(mx,mz)*.62*fade*microFade;vec3 n=normalize(normalW+vec3(grad.x,0.,grad.y));vec3 v=normalize(cameraPosition-wp),l=normalize(vec3(12.,12.,-48.)-wp);float fres=.025+.975*pow(1.-max(dot(n,v),0.),5.);vec3 refl=reflect(-v,n);float a=dot(refl.xz,refl.xz),b=dot(wp.xz,refl.xz),c=dot(wp.xz,wp.xz)-5184.;float dist=(-b+sqrt(max(0.,b*b-a*c)))/max(a,.001);vec3 hit=wp+refl*dist;vec2 uv=vec2(acos(cos(atan(hit.x,-hit.z)+1.570796327))/3.141592654,(hit.y-panoramaHeight)/100.+.5);vec3 reflection=texture2D(panorama,clamp(uv,vec2(.005),vec2(.995))).rgb;vec3 base=mix(waterColor,reflection*.94,clamp(.24+fres*.7,0.,.96));base+=vec3(.018,.028,.035)*sin(wp.x*.23+wp.z*.18)*.3;float spec=max(dot(n,normalize(l+v)),0.);float sparkle=pow(spec,180.)*.38+pow(spec,42.)*.13;base+=vec3(1.,.89,.72)*sparkle;float tiny=pow(max(dot(n,normalize(vec3(-.2,1.,.3)+v)),0.),145.);base+=vec3(.24,.49,.46)*tiny*.16;float d=distance(wp.xz,center),age=time-impact;float ring=exp(-pow(d-age*2.2,2.)*14.)*exp(-age*.8);if(age>0.)base+=vec3(.3,.6,.51)*ring*.45;float fog=1.-exp(-length(wp-cameraPosition)*.009);base=mix(base,mistColor,fog);gl_FragColor=vec4(base,.98-.32*(1.-smoothstep(.2,1.6,d)));}` }));water.rotation.x=-Math.PI/2;scene.add(water);
    const textures=[];
    function texture(kind){const c=document.createElement('canvas');const size=kind==='stone'?1024:512;c.width=c.height=size;const ctx=c.getContext('2d'),im=ctx.createImageData(size,size);for(let y=0;y<size;y++)for(let x=0;x<size;x++){let i=(y*size+x)*4;let h=Math.sin(x*127.1+y*311.7)*43758.5453;let n=h-Math.floor(h);let v=kind==='carbon'?38+12*Math.sin((x+y)*.48)*Math.sin((x-y)*.48)+n*5:kind==='cork'?155+Math.sin(x*.042+y*.11)*12+Math.sin(x*.61+y*.14)*5+n*17-(n>.967?50:0):132+Math.sin(x*.022+Math.sin(y*.017)*2)*18+Math.sin(x*.071-y*.049)*10+Math.sin(x*.21+y*.17)*5+n*12;im.data[i]=v;im.data[i+1]=v;im.data[i+2]=v;im.data[i+3]=255;}ctx.putImageData(im,0,0);const tex=new T.CanvasTexture(c);tex.anisotropy=Math.min(16,renderer.capabilities.getMaxAnisotropy());tex.wrapS=tex.wrapT=T.RepeatWrapping;tex.repeat.set(kind==='carbon'?2:1,kind==='carbon'?8:1);textures.push(tex);return tex;}
    ...
    const stone=shoreTexture('granite-v2.png'),stoneRelief=shoreTexture('granite-v2.png',false),leafTexture=shoreTexture('reed-v2.png'),carbon=craft?.carbon||texture('carbon'),cork=craft?.cork||texture('cork');stone.repeat.set(2,1);stoneRelief.repeat.copy(stone.repeat);
    ...
    rockMat.onBeforeCompile=shader=>{shader.vertexShader='varying vec3 shoreWorld;\n'+shader.vertexShader;shader.vertexShader=shader.vertexShader.replace('#include <worldpos_vertex>','#include <worldpos_vertex>\nvec4 rockPosition=vec4(transformed,1.);\n#ifdef USE_INSTANCING\nrockPosition=instanceMatrix*rockPosition;\n#endif\nshoreWorld=(modelMatrix*rockPosition).xyz;');shader.fragmentShader='varying vec3 shoreWorld;\n'+shader.fragmentShader;shader.fragmentShader=shader.fragmentShader.replace('#include <map_fragment>',`#ifdef USE_MAP\nvec3 weights=pow(abs(normalize(cross(dFdx(shoreWorld),dFdy(shoreWorld)))),vec3(4.));weights/=max(dot(weights,vec3(1.)),.001);vec3 q=shoreWorld*1.3;vec3 grain=texture2D(map,q.yz).rgb*weights.x+texture2D(map,q.xz).rgb*weights.y+texture2D(map,q.xy).rgb*weights.z;diffuseColor.rgb*=grain;\n#endif`);shader.fragmentShader=shader.fragmentShader.replace('#include <color_fragment>','#include <color_fragment>\nfloat wet=1.-smoothstep(-.025,.19,shoreWorld.y);diffuseColor.rgb*=mix(1.,.55,wet);');shader.fragmentShader=shader.fragmentShader.replace('#include <roughnessmap_fragment>','#include <roughnessmap_fragment>\nroughnessFactor=mix(clamp(roughnessFactor+.07*sin(shoreWorld.x*37.+shoreWorld.z*29.),.65,.95),.23,1.-smoothstep(-.025,.19,shoreWorld.y));');};
    // Different eroded strata, rather than copies of a uniformly lumpy sphere.
    const rockVariants=Array.from({length:5},(_,variant)=>{const g=rockGeo.clone(),a=g.attributes.position;for(let i=0;i<a.count;i++){const x=a.getX(i),y=a.getY(i),z=a.getZ(i);const layer=Math.sin(y*9+variant)*.035,fracture=Math.sin(x*5.1+z*3.7+variant*2)*.045;const flat=Math.sign(y)*Math.pow(Math.abs(y),.72);a.setXYZ(i,x*(1+layer+fracture),flat*(.83+variant*.035),z*(1-layer));}g.computeVertexNormals();return g;});
    for(let i=0;i<24;i++){const m=new T.Mesh(rockVariants[i%5],rockMat),size=.35+(i%5)*.14;m.position.set((i%2?1:-1)*(2.7+i*.105),-.09,4.8-i*.38);m.scale.set(size*1.5,size*.65,size);m.rotation.set(i*.31,.7*i,.1*Math.sin(i));shore.add(m);}
    for(let i=0;i<3;i++){const m=new T.Mesh(rockVariants[(i+2)%5],rockMat);m.position.set(i===0?-2.25:i===1?2.55:-3.05,.10,i===2?-1.2:.25);m.scale.set(i===2?.7:1.15,i===2?.42:.72,.82);m.rotation.y=i*1.7;shore.add(m);}
    ...
    const shaftMat=new T.MeshPhysicalMaterial({color:0xb8c2ca,clearcoat:.85,clearcoatRoughness:.22,map:carbon,bumpMap:carbon,bumpScale:.0012,metalness:.4,roughness:.29}),gold=new T.MeshStandardMaterial({color:0xd7bb7c,metalness:.75,roughness:.25});
    const shaft=new T.Mesh(new T.CylinderGeometry(.009,.043,3.7,40,24),shaftMat);shaft.position.y=2.1;rod.add(shaft);const shaftRest=shaft.geometry.attributes.position.array.slice();let rodFlex=0;
    const grip=new T.Mesh(new T.CylinderGeometry(.060,.068,.65,32,8),new T.MeshStandardMaterial({color:0xe5c49a,map:cork,bumpMap:cork,bumpScale:.003,roughness:.78}));grip.position.y=.38;rod.add(grip);
    ...
    const paintCanvas=document.createElement('canvas');paintCanvas.width=512;paintCanvas.height=1024;const pc=paintCanvas.getContext('2d');const pg=pc.createLinearGradient(0,0,512,0);pg.addColorStop(0,'#c49955');pg.addColorStop(.32,'#cda762');pg.addColorStop(.65,'#c8a15c');pg.addColorStop(1,'#c49955');pc.fillStyle=pg;pc.fillRect(0,0,512,1024);for(let i=0;i<320;i++){pc.strokeStyle='rgba(93,48,16,.08)';pc.beginPath();pc.moveTo(i*1.6,0);pc.bezierCurveTo(i*1.6+3,300,i*1.6-3,750,i*1.6,1024);pc.stroke();}for(const y of [100,116,872,890]){pc.fillStyle='#e6d8ab';pc.fillRect(0,y,512,5);}const floatPaint=new T.CanvasTexture(paintCanvas);floatPaint.colorSpace=T.SRGBColorSpace;floatPaint.anisotropy=Math.min(8,renderer.capabilities.getMaxAnisotropy());textures.push(floatPaint);
    const lacquer=new T.MeshPhysicalMaterial({color:0xffffff,map:floatPaint,roughness:.26,clearcoat:1,clearcoatRoughness:.12});
    const profile=[[-.28,.012],[-.20,.045],[-.09,.070],[.03,.074],[.14,.052],[.22,.018]].map(([y,r])=>new T.Vector2(r,y));
    const body=new T.Mesh(new T.LatheGeometry(new T.SplineCurve(profile).getPoints(80),64),lacquer);float.add(body);
    const stem=new T.Mesh(new T.CylinderGeometry(.009,.013,.74,20),new T.MeshPhysicalMaterial({color:0xf2eac6,roughness:.3,clearcoat:.6}));stem.position.y=.35;float.add(stem);for(let i=0;i<20;i++){const tick=new T.Mesh(new T.CylinderGeometry(.011,.011,.004,16),new T.MeshStandardMaterial({color:0x242d32,roughness:.4}));tick.position.y=.20+i*.028;float.add(tick);}
    ...
    const baitTexture=texture('cork');baitTexture.repeat.set(4,1);const baitMat=new T.MeshPhysicalMaterial({color:0xb16d58,map:baitTexture,bumpMap:baitTexture,bumpScale:.0008,roughness:.48,clearcoat:.35});
    const baitPath=new T.CatmullRomCurve3([new T.Vector3(.01,-.04,.01),new T.Vector3(.045,-.025,.008),new T.Vector3(.07,-.055,.012),new T.Vector3(.055,-.09,.015)]);hook.add(new T.Mesh(new T.TubeGeometry(baitPath,48,.012,16,false),baitMat));for(let i=1;i<18;i++){const u=i/18,segment=new T.Mesh(new T.TorusGeometry(.0122,.001,5,16),baitMat);segment.position.copy(baitPath.getPoint(u));segment.quaternion.setFromUnitVectors(new T.Vector3(0,0,1),baitPath.getTangent(u).normalize());hook.add(segment);}
    ...
    function feedback(stage,options={}){finishFeedback(true);host.dataset.feedback=stage;
      ...
      phase='waiting';host.dataset.phase=stage;const duration=reduced.matches?.35:stage==='bite'?1.8:stage==='catch'?6.8:4.2;
      caughtFish.userData.species?.(options.name);caughtFish.scale.setScalar(T.MathUtils.clamp(Number(options.size)||1,.65,1.65));
    ...
    const resize=new ResizeObserver(()=>{if(!host.isConnected)return;const w=host.clientWidth,h=host.clientHeight;const desktop=matchMedia("(min-width:1000px)").matches;renderer.setPixelRatio(Math.min(devicePixelRatio,desktop?2:1.75,Math.sqrt((desktop?8000000:2400000)/Math.max(1,w*h))));renderer.setSize(w,h,false);camera.aspect=w/h;camera.updateProjectionMatrix();});resize.observe(host);
    ...
    function animate(now){if(dead)return;frame=requestAnimationFrame(animate);let dt=Math.min((now-last)/1000,.05);last=now;if(document.hidden||!host.isConnected)return;t+=dt;leafWind.value=reduced.matches?0:t;uniforms.time.value=t;uniforms.waveScale.value=config.wave*(reduced.matches?.25:1);
      ...
      if(feedbackState){const f=feedbackState,p=Math.min(1,(t-f.start)/f.duration),motion=reduced.matches?.25:1;
        ...
        if(f.stage==='catch'){
          ...
          caughtFish.position.set(T.MathUtils.lerp(target.x,-.65,tow)+lateral,
          ...
          const heading=-Math.atan2(2.2-target.z,-.65-target.x);
          caughtFish.rotation.order='YXZ';
          caughtFish.rotation.set((Math.sin(p*29)*.35+Math.sin(p*13)*.16)*struggle,heading+lift*.65+Math.sin(p*25)*.22*struggle,
      ...
      const wantedFlex=hooked?.18:phase==='casting'?Math.sin(age*9)*Math.exp(-Math.max(0,age-.55)*2)*.16:Math.sin(t*1.3)*.009;rodFlex+=(wantedFlex-rodFlex)*Math.min(1,dt*12);const shaftPoints=shaft.geometry.attributes.position;for(let i=0;i<shaftPoints.count;i++){const u=(shaftRest[i*3+1]+1.85)/3.7;shaftPoints.setZ(i,shaftRest[i*3+2]+rodFlex*u*u);}shaftPoints.needsUpdate=true;tip.position.z=rodFlex;for(const part of rod.children){if(part.geometry?.type==='TorusGeometry'&&part.position.y>.79)part.position.z=-.047+rodFlex*Math.pow((part.position.y-.25)/3.7,2);}tip.getWorldPosition(tipWorld);const lineEnd=hooked?hookEnd:float.position;const a=lineGeo.attributes.position;for(let i=0;i<=40;i++){let p=i/40;a.setXYZ(i,T.MathUtils.lerp(tipWorld.x,lineEnd.x,p),T.MathUtils.lerp(tipWorld.y,lineEnd.y+(hooked?0:.2),p)-Math.sin(p*Math.PI)*(phase==='casting'?.65:slack),T.MathUtils.lerp(tipWorld.z,lineEnd.z,p));}a.needsUpdate=true;
